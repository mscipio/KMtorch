...
*******************************************************************************************************************************/
__device__ void bicomp_3expIF(unsigned int idx, float *aux_par, float *inputfuns, float *IF, float *times, float *func, float *jac, float *dk, float *mask);
...
{ 
    ...
    bicomp_3expIF(idx, aux_par, inputfuns, IF, times, func, jac, dk, mask);
    //__syncthreads();
}
...
{     
    ...
    if (idx < ${N}) {
        ...
        for (uint ii=0; ii<=2; ii+=2) {   //i = 1:2:4 % 2 compartiments
            ...
            for (uint tt=0; tt<${T}; ++tt) {
                if (times[tt]>=inputfuns[0]) {
                    TAC[tt] += p[ii] * (sum[tt] + ((inputfuns[1]*(times[tt]-inputfuns[0]))/delta0) *exp(inputfuns[4]*(times[tt]-inputfuns[0])))* exp(-dk[0]*(times[tt]-inputfuns[0]));        
                    jac[INDEX_JAC_TIME(idx,tt,ii+1)] = -(1-aux_par[INDEX_PARAM(idx,0)]) * (Jb[tt]
                                                                                              + ((inputfuns[1]*(times[tt]-inputfuns[0]))/delta0) 
                                                                                                   *exp(inputfuns[4]*(times[tt]-inputfuns[0]))
                                                                                         )* exp(-dk[0]*(times[tt]-inputfuns[0]));
                    jac[INDEX_JAC_TIME(idx,tt,ii+2)] = -(1-aux_par[INDEX_PARAM(idx,0)]) * (p[ii] * (Jl[tt]
                                                                                                        - inputfuns[1] * (times[tt]-inputfuns[0]) * (1.0f / (delta0*delta0)) 
                                                                                                              * ( exp(inputfuns[4]*(times[tt]-inputfuns[0]))  + exp(-p[ii+1]*(times[tt]-inputfuns[0])) )
                                                                                                        + 2*inputfuns[1] * (1.0f / (delta0*delta0*delta0))
                                                                                                              * ( exp(inputfuns[4]*(times[tt]-inputfuns[0]))  - exp(-p[ii+1]*(times[tt]-inputfuns[0])) )
                                                                                                  )
                                                                                         )* exp(-dk[0]*(times[tt]-inputfuns[0]));
                    ...
                    jac[INDEX_JAC_TIME(idx,tt,ii+2)] = (1-aux_par[INDEX_PARAM(idx,0)]) * (p[ii] * (Jl[tt] 
                                                                    + ( exp(-p[ii+1]*(times[tt]-inputfuns[0]))-exp(inputfuns[4]*(times[tt]-inputfuns[0]))) 
                                                                           * (inputfuns[1] *(times[tt]-inputfuns[0]) * (1.0f / (delta0*delta0)) 
                                                                    + 2*inputfuns[1] * (1.0f / (delta0*delta0*delta0))) ));*/
