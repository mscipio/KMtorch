...
__global__ void gaussian_MRF_prior(float *k4D, float *prior, float *beta, float *gamma, float *threshold)
{
    ...
    if (idx < ${N}) {    
        ...
        for (uint dd=0; dd<${D}; dd++) {
            ...
            sum += (m+gamma[dd])*k4D[INDEX_4D(x, y, z, dd)] ;
            prior[INDEX_PARAM(idx,dd)] = beta[dd]*sum;
            //prior[INDEX_PARAM(idx,dd)] = 1e8*sum;

        }
    }
}

__global__ void sparsity_prior(float *k4D, float *prior, float *beta, float *gamma, float *threshold)
{
    // Obtain the linear index corresponding to the current thread:
    unsigned int idx = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if (idx < ${N}) {    
	    ...
	    sum += gamma[dd]  *k4D[INDEX_4D(x, y, z, dd)];
            prior[INDEX_PARAM(idx,dd)] -= beta[dd]*sum;
